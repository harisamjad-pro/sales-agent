...
const D = async ({ query }: { query: string }) => {
  const cookieHeader = await (await cookies()).getAll().map(c => `${c.name}=${c.value}`).join("; ");
  const { data, error } = await AllDomains(query, cookieHeader);
  return error ? <p className="bg-red-600 font-medium text-sm text-white px-4 py-2 rounded-lg w-fit">{error}</p> : <List data={data ?? []} />
}
